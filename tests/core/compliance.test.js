            ...
            describe(`Standard: ${file}`, () => {
                testCases.forEach((testCase) => {
                    it(`${testCase.id}: ${testCase.description}`, () => {
                        const jsonql = new core_1.JSONQL(testCase.schema); // Pass schema if available
        ...
        issueFolders.forEach(folder => {
            ...
            if (fs.existsSync(testFile)) {
                ...
                describe(`Issue: ${folder}`, () => {
                    testCases.forEach((testCase) => {
                        it(`${testCase.id}: ${testCase.description}`, () => {
                            const jsonql = new core_1.JSONQL(schema || testCase.schema);
                            if (testCase.valid !== false) { // Default to valid if not specified
                                expect(() => {
                                    jsonql.parse(testCase.query);
                                }).not.toThrow();
                            }
    ...
    if (fs.existsSync(securityDir)) {
        ...
        files.forEach(file => {
            ...
            describe(`Security: ${file}`, () => {
                testCases.forEach((testCase) => {
                    it(`${testCase.id}: ${testCase.description}`, () => {
                        const jsonql = new core_1.JSONQL(testCase.schema);
                        if (testCase.valid) {
                            expect(() => {
                                jsonql.parse(testCase.query);
                            }).not.toThrow();
                ...
                describe(`Permissions: ${file}`, () => {
                    testCases.forEach((testCase) => {
                        it(`${testCase.id}: ${testCase.description}`, () => {
                            ...
                            const jsonql = new core_1.JSONQL(schema, testCase.tableName);
                            if (testCase.valid) {
                                expect(() => {
                                ...
                                try {
                                    ...
                                    if (testCase.errorCode) {
                                        const hasCode = result.validation.errors.some((e) => e.code === testCase.errorCode);
                                        if (!hasCode) {
                                            throw new Error(`Expected error code ${testCase.errorCode} but got ${JSON.stringify(result.validation.errors)}`);
                                        }
                                    }
                                }
                                catch (e) {
                                    ...
                                    if (e.message === 'Expected validation to fail but it passed' || e.message.startsWith('Expected error code')) {
                                        throw e;
                                    }
                                }
                            }
