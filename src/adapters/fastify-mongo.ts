...
export type JsonqlFastifyMongoOptions = MongoAdapterOptions<FastifyRequest>;

/**
 * Fastify adapter for JSONQL backed by MongoDB.
 *
 * Mirrors {@link FastifyAdapter} but executes against a MongoDB database.
 */
export class FastifyMongoAdapter
  extends MongoBaseHandler<FastifyRequest>
  implements FrameworkAdapter<FastifyRequest>
{
  constructor(options: JsonqlFastifyMongoOptions) {
    super(options);
  }
...
export const jsonqlFastifyMongo = fp(plugin);
